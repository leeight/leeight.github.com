...
goog.provide('base.OneToArrayParamAdapter');
 ...
 * @param {string} fromParam 需要转换的参数名称.
 */
base.OneToArrayParamAdapter = function(delimiter, fromParam) {
    this.delimiter = delimiter;
    this.fromParam = fromParam;
};
 ...
 * @inheritDoc
 */
base.OneToArrayParamAdapter.prototype.processParam = function(param) {
    var regex = new RegExp(this.fromParam + '=(.+)', 'g'),
        params = param.split('&'),
        match, value, values, i;
    for (i = params.length - 1; i >= 0; i--) {
 ...
 * @inheritDoc
 */
base.OneToArrayParamAdapter.prototype.processObject = function(obj) {
    var newValue = [],
        value,
